export default class Siema {
  ...
  generateBullets(){
    if(this.config.bullets){
      let target = document.querySelector(this.config.selector);
      let elementsLength = target.querySelectorAll(this.config.childClass).length;

      //Creates a contianer for our bullets and injects it in position
      let bulletContainer = document.createElement('div');
      bulletContainer.classList.add('bullet__container');
      target.closest(this.config.selector).insertAdjacentElement('beforeend', bulletContainer);
      let container = document.querySelector('.bullet__container');
      //Generates bullet for each slide
      for(let i = 0; i < elementsLength -1; i += 1){
        let newBullet = document.createElement('div');
        newBullet.classList.add('bullet');
        newBullet.textContent = `page: ${i + 1}`;
        if(i === 0){
          newBullet.classList.add('first');
        } 
        if(i === elementsLength -1){
          newBullet.classList.add('last');
        }
        container.insertAdjacentElement('beforeend', newBullet);
      };

      /*Adds click on the bullets parent to avoid 
      creating n°+infinity listeners since events bubbles up [event bubbling]*/
      container.addEventListener('click', (e) => {
        let first = false;
        let last = false;
        let curBullet = e.target;
        if(curBullet.classList.contains('first') && !first){
          this.prev();
          first = true;
        }
        if(curBullet.classList.contains('last') && !last){
          this.next();
          last = true;
        }
        this.next();
